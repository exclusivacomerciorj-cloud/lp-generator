    ...
    try {
      ...
      const response = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({
          ...
          messages: [{
            role: 'user',
            content: `Voce e especialista em landing pages imobiliarias. Recebera um HTML completo e uma instrucao de ajuste. Responda APENAS com o HTML completo modificado. Sem explicacoes, sem markdown. Apenas HTML bruto comecando com <!DOCTYPE html>.\n\nHTML atual:\n${active.html}\n\nINSTRUCAO:\n${refinePrompt}\n\nGere o HTML completo atualizado:`,
          }],
        }),
      });
      if (!response.ok) {
    ...
    <div style={{ minHeight: '100vh', background: '#0f1923', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ borderBottom: '1px solid rgba(255,255,255,0.08)', padding: '12px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          ...
          <button onClick={handlePublish} disabled={publishing} style={{ padding: '8px 16px', background: publishing ? 'rgba(255,255,255,0.1)' : '#4ade80', color: publishing ? 'rgba(255,255,255,0.3)' : '#0f1923', border: 'none', borderRadius: 8, fontSize: 12, fontWeight: 700, cursor: publishing ? 'not-allowed' : 'pointer' }}>
      ...
      {/* URL publicada */}
      {publishedUrl && (
        <div style={{ padding: '10px 20px', background: 'rgba(34,197,94,0.08)', borderBottom: '1px solid rgba(34,197,94,0.2)', display: 'flex', alignItems: 'center', gap: 12 }}>
          <span style={{ fontSize: 12, color: '#4ade80' }}>✓ Publicado:</span>
          <a href={publishedUrl} target="_blank" rel="noreferrer" style={{ fontSize: 12, color: '#4ade80', textDecoration: 'underline' }}>{publishedUrl}</a>
      ...
      {/* Erro publicacao */}
      {publishError && (
        <div style={{ padding: '10px 20px', background: 'rgba(239,68,68,0.08)', borderBottom: '1px solid rgba(239,68,68,0.2)', fontSize: 12, color: '#f87171' }}>
          ⚠ {publishError}
        </div>
      )}
      ...
      <div style={{ borderBottom: '1px solid rgba(255,255,255,0.06)', padding: '8px 20px', display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
        {(['desktop', 'mobile'] as const).map(m => (
          <button key={m} onClick={() => setPreviewMode(m)} style={{ padding: '5px 14px', borderRadius: 6, fontSize: 11, background: previewMode === m ? 'rgba(255,255,255,0.1)' : 'none', border: `1px solid ${previewMode === m ? 'rgba(255,255,255,0.2)' : 'transparent'}`, color: previewMode === m ? '#fff' : 'rgba(255,255,255,0.3)', cursor: 'pointer' }}>
            {m === 'desktop' ? 'Desktop' : 'Mobile'}
          </button>
        ))}
        <div style={{ flex: 1 }} />
        <button onClick={() => download(active)} style={{ padding: '6px 14px', borderRadius: 6, fontSize: 11, background: '#c9a84c', color: '#0f1923', border: 'none', fontWeight: 700, cursor: 'pointer' }}>
          Baixar HTML
        </button>
      </div>
