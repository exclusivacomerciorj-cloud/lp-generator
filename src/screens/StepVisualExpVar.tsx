      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          {[
            ...
            { label: 'Cor Secundária / CTA', key: 'secondaryColor', value: form.secondaryColor },
          ].map(c => (
            <div key={c.key}>
        ...
        <div style={{ borderRadius: 14, padding: 24, background: `linear-gradient(135deg, ${form.primaryColor}33, ${form.primaryColor}66)`, border: '1px solid rgba(255,255,255,0.08)' }}>
          ...
          <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.5)', marginBottom: 20 }}>{form.location || 'Localização'}</div>
        ...
        <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10 }}>
            {[
              { value: 'clean', label: '✦ Clean', desc: 'Minimalista, muito espaço, tipografia forte' },
              { value: 'sofisticado', label: '◈ Sofisticado', desc: 'Premium, elegante, gradientes sutis' },
              { value: 'moderno', label: '◉ Moderno', desc: 'Dinâmico, bold, elementos gráficos' },
            ].map(opt => (
...
export function StepExperience({ form, updateForm, goTo }: Props) {
  ...
  return (
    <StepLayout title="Experiência & UX" subtitle="Interatividade e elementos da LP" currentStep="experience" goTo={goTo}
      onBack={() => goTo('visual')} onNext={() => goTo('variations')}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        <CheckOpt checked={form.animations} onChange={v => updateForm({ animations: v })} label="Ativar animações" desc="Fade-in nos elementos durante o scroll, hover nos botões" />
        <CheckOpt checked={form.arrows} onChange={v => updateForm({ arrows: v })} label="Usar setas direcionais" desc="Setas que guiam o olhar do usuário para os CTAs" />
        <CheckOpt checked={form.hasVideo} onChange={v => updateForm({ hasVideo: v })} label="Incluir vídeo" desc="Embed YouTube/Vimeo — mantém a LP leve e rápida" />
        ...
        {form.hasVideo && (
          <div style={{ marginLeft: 16, padding: 20, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 12, display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <div>
              <label style={labelStyle}>Tipo de Vídeo</label>
...
export function StepVariations({ form, updateForm, goTo }: Props) {
  return (
    <StepLayout title="Variações Estratégicas" subtitle="Multiplique seus resultados com múltiplas versões da LP" currentStep="variations" goTo={goTo}
      onBack={() => goTo('experience')} onNext={() => goTo('generate')} nextLabel="Gerar LPs →">
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        <div style={{ background: 'rgba(201,168,76,0.08)', border: '1px solid rgba(201,168,76,0.25)', borderRadius: 12, padding: '14px 18px', fontSize: 12, color: '#c9a84c', marginBottom: 6 }}>
          💡 Cada variação gera uma LP independente. Mais versões = mais dados = menor CPL no Google Ads.
        </div>
        <CheckOpt checked={form.withPrice} onChange={v => updateForm({ withPrice: v })} label="LP com preço" desc="Versão padrão exibindo o valor — atrai leads mais qualificados" />
        <CheckOpt checked={form.withoutPrice} onChange={v => updateForm({ withoutPrice: v })} label="LP sem preço" desc="Esconde o preço — maior volume de leads para qualificar no atendimento" />
        <CheckOpt checked={form.investorVersion} onChange={v => updateForm({ investorVersion: v })} label="Versão Investidor" desc="Copy focada em retorno, valorização e oportunidade de mercado" />
        <CheckOpt checked={form.housingVersion} onChange={v => updateForm({ housingVersion: v })} label="Versão Moradia" desc="Copy focada em qualidade de vida, família e bem-estar" />
        <CheckOpt checked={form.headlineVariation} onChange={v => updateForm({ headlineVariation: v })} label="Variação de headline (A/B)" desc="Gera 2 versões do topo com headlines diferentes para teste" />
      </div>
    </StepLayout>
  );
}
